...
required_packages <- c("asreml", "ggplot2", "lattice", "gridExtra", "dplyr", "plyr", "ggrepel", "tidyr","ggfortify", "missMDA", "ggpubr", "pheatmap", "kableExtra", "DT", "configr", "remotes", "plotly")
...
```{r distribution_plots, fig.width = 6, fig.height = 6, dpi = 300}
...
select.environment <- summS[!is.na(summS$sel_model), "Environment"]
...
```{r model_selection}
create_dt(merge(summS[,c("Environment", "sel_model")], mod.list, by.x="sel_model", by.y="Model", sort=FALSE))
```
...
BMSSummary$MeanSED         <- aggregate(predS$std.error, list(predS$Environment), mean)$x
BMSSummary$MinSED          <- aggregate(predS$std.error, list(predS$Environment), min)$x
BMSSummary$MaxSED          <- aggregate(predS$std.error, list(predS$Environment), max)$x
BMSSummary$MeanLSD         <- bms.summS$LSD
BMSSummary$MinLSD          <- aggregate(predS$std.error, list(predS$Environment), min)$x * 1.96
BMSSummary$MaxLSD          <- aggregate(predS$std.error, list(predS$Environment), max)$x * 1.96
...
valid.env <- summS[(summS$Type != "AUGMENTED" & summS$heritVC > as.numeric(config$met$heritVC.limit)) |
